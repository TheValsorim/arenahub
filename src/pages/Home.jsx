...
import { Radio, Trophy, ChevronRight, Zap, Plus } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    const loadHomeData = async () => {
      ...
      const firstError =
        liveStreamsResult.error ||
        upcomingStreamsResult.error ||
        competitionsResult.error ||
        categoriesResult.error ||
        disciplinesResult.error;

      if (firstError) {
        console.error("Home page load error:", firstError);
        setError(firstError.message || "Could not load home page data.");
      }

      setLiveStreams(liveStreamsResult.data || []);
      setUpcomingStreams(upcomingStreamsResult.data || []);
    ...
    <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="arena-card border-l-4 border-l-ember p-5 flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Enable Creator mode to stream, or Organizer mode to run
              competitions.
            </p>
          </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-5">
          ...
          <Link
            to="/streams"
            className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-arena-fast"
          >
        ...
        {liveStreams.length > 0 ? (
          ...
          <div className="arena-card p-10 text-center">
            ...
            {profile?.is_creator ? (
              <Link
                to="/go-live"
                className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-ember text-white rounded-lg text-sm font-medium"
              >
      ...
      <section>
        ...
        {competitions.length > 0 ? (
          ...
          <div className="arena-card p-10 text-center">
            ...
            />
            <p className="text-muted-foreground font-medium">
              No open competitions yet
            </p>

            {profile?.is_organizer ? (
              <Link
                to="/create-competition"
                className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-foreground text-background rounded-lg text-sm font-medium"
              >
                <Plus size={14} /> Create first competition
              </Link>
            ) : (
              <p className="text-sm text-muted-foreground/70 mt-1">
                Come back soon — competitions are being set up
              </p>
            )}
          </div>
        )}
