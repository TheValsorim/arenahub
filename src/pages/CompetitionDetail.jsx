...
export default function CompetitionDetail() {
  ...
  const loadCompetition = async () => {
    ...
    if (compError) {
      console.error("Competition load error:", compError);
      setError(compError.message || "Could not load competition.");
      setLoading(false);
      return;
    }
    ...
    if (firstError) {
      console.error("Competition detail load error:", firstError);
      setError(firstError.message || "Could not load competition details.");
    }
    ...
    if (user) {
      const myReg = (registrationsResult.data || []).find(
        (r) => r.user_id === user.id
      );
  ...
  const handleRegister = async () => {
    if (!user) {
      sessionStorage.setItem("arena_intended_dest", `/competitions/${id}`);
    ...
    if (insertError) {
      ...
      setError(insertError.message || "Could not register.");
  ...
  if (!competition) {
    return (
      <div className="text-center py-24">
        ...
        <Link
          to="/competitions"
          className="mt-4 inline-block text-sm text-ember"
        >
  ...
  const canRegister =
    ["registration_open"].includes(competition.status) &&
    !regStatus &&
    !isOrganizer;
  ...
  return (
    <div className="w-full max-w-full space-y-4">
      <Link
        to="/competitions"
        className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-arena-fast"
      >
      ...
      <div className="relative rounded-xl overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white font-heading leading-tight mb-3">
            {competition.name}
          </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Format",
            value: competition.format?.replace(/_/g, " ") || "—",
            icon: BarChart3,
          },
          {
            label: "Starts",
          ...
          {
            label: "Location",
            value: competition.location_type || "—",
            icon: MapPin,
          },
          ...
          {TABS.map((t) => (
            ...
            >
              ...
              {t.id === "live" && liveStreams.length > 0 && (
                <span className="ml-1 px-1.5 py-0.5 text-xs bg-live-light text-live rounded-full font-bold">
                  {liveStreams.length}
                </span>
              )}

              {t.id === "participants" && approvedCount > 0 && (
                <span className="ml-1 px-1.5 py-0.5 text-xs bg-secondary text-muted-foreground rounded-full">
                  {approvedCount}
                </span>
              )}
            </button>
                ...
                <div className="space-y-3">
                  ...
                  {[
                    ...
                    competition.location_name && {
                      label: "Venue",
                      value: competition.location_name,
                    },
                    {
                      label: "Team Size",
                    ...
                    },
                    {
                      ...
                      value: competition.max_participants || "Unlimited",
                    },
                    competition.registration_deadline && {
                      label: "Reg. Deadline",
                      value: formatDate(competition.registration_deadline),
                    },
                    {
                      label: "Start Date",
                      value: formatDate(competition.start_date),
                    ...
                    .map(({ label, value }) => (
                      <div
                        key={label}
                        className="flex justify-between text-sm py-1.5 border-b border-border/50 gap-4"
                      >
                    ...
                    {competition.max_participants && (
                      <>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {approvedCount}/{competition.max_participants} spots
                          filled
                        </p>
                      </>
                    )}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">
                              {reg.team_id ? "Team Entry" : `Player #${i + 1}`}
                            </p>
              ...
              {matches.length === 0 ? (
                    ...
                    <div key={round.id}>
                      ...
                      {roundMatches.map((match) => (
                        <Link
                          ...
                          className="block mb-2 p-4 bg-secondary rounded-lg hover:bg-secondary/80 transition-arena-fast group"
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3 flex-1">
                              <div className="text-sm font-semibold">
                                {match.team_a_id ? "Team A" : "TBD"}
                              </div>
                              <div className="px-2 py-0.5 bg-border rounded text-xs font-bold">
                                VS
                              </div>
                              <div className="text-sm font-semibold">
                                {match.team_b_id ? "Team B" : "TBD"}
                              </div>
                            </div>
                ...
                matches.map((match) => (
                  <Link
                    key={match.id}
                    to={`/match-center/${match.id}`}
                    className="block p-4 bg-secondary rounded-lg hover:bg-secondary/80 transition-arena-fast"
                  >
                    <div className="flex items-center justify-between">
            ...
            <div>
              {liveStreams.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  />
                  <p className="text-muted-foreground">
                    No streams live for this competition
                  </p>

                  {competition.allow_player_streams && profile?.is_creator && (
                    <Link
                      to={`/go-live?competition=${id}`}
                      className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-ember text-white rounded-lg text-sm"
                    >
            ...
            <div>
              {streams.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  />
                  <p className="text-muted-foreground">
                    No streams linked to this competition yet
                  </p>
                </div>
              )}
            </div>
