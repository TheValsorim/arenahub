...
export default function GoLive() {
  ...
  const loadData = async () => {
    ...
    const [categoriesResult, disciplinesResult, competitionsResult] =
      await Promise.all([
        supabase
          .from("categories")
          .select("*")
    ...
    if (firstError) {
      console.error("GoLive load error:", firstError);
      setPageError(firstError.message || "Could not load streaming data.");
    }
    ...
    setChannel(syntheticChannel);

    setPlan({
      name: "Free",
      monthly_stream_hours_limit: 20,
    });
 ...
 const handleGoLive = async () => {
  ...
  try {
    ...
    const roomName = "arenahub_obs_" + shareId;
    const playbackUrl = `${window.location.origin}/watch/${shareId}`;

    const { data: sessionData } = await supabase.auth.getSession();
    const accessToken = sessionData?.session?.access_token;

    if (!accessToken) {
      throw new Error("You must be logged in to create OBS credentials.");
    }
    ...
    const ingressData = await response.json();

    if (!response.ok) {
      throw new Error(
        ingressData.error || "Could not create LiveKit RTMP ingress."
      );
    }
    ...
    const { data: stream, error } = await supabase
      ...
      .insert({
        ...
        livekit_ingress_id: ingressData.ingressId,
        livekit_room_name: ingressData.roomName || roomName,

        stream_key: ingressData.streamKey,
        rtmp_url: ingressData.url,
        playback_url: playbackUrl,
        public_watch_url: `/watch/${shareId}`,
    ...
    console.error("OBS stream create error:", err);
    setPageError(err.message || "Could not create OBS stream.");
  } finally {
    setCreating(false);
  }
};
  ...
  const getCameraStream = async (facing) => {
    if (!navigator.mediaDevices?.getUserMedia) {
      throw Object.assign(new Error("Browser not supported"), {
  ...
  const startCameraPreview = async () => {
    ...
    try {
      ...
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        setCameraError(
          "Camera permission is required to start phone streaming. Please allow camera access in your browser settings."
        );
      } else if (err.name === "NotFoundError") {
        setCameraError("No camera found on this device.");
      } else if (err.name === "NotSupportedError") {
        setCameraError(
          "Your browser does not support camera streaming. Try Chrome or Safari on mobile."
        );
      } else {
        setCameraError(
  ...
  const switchCamera = async () => {
    ...
    try {
      ...
      setCameraWarning(
        "Could not switch camera: " + (err.message || "Unknown error")
      );
    }
  };
  ...
  const handleStartPhoneStream = async () => {
    if (!phoneForm.title.trim() || !localStreamRef.current || !user) return;
    ...
    try {
      ...
      console.error("Phone stream create error:", err);
      setPageError(err.message || "Could not start phone stream.");
    } finally {
      setCreating(false);
    }
  };
  ...
  const phoneFilteredDisciplines = phoneForm.category_id
    ? disciplines.filter((d) => d.category_id === phoneForm.category_id)
    : disciplines;
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Configure and start your stream session
        </p>
      </div>
      ...
      {plan && (
        <div
          className={`arena-card p-4 flex items-center justify-between ${
            isAtLimit
        ...
        >
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {isUnlimited
                ...
                : `${hoursUsed.toFixed(1)} / ${hoursLimit}h used this month`}
            </p>
          </div>
          ...
          {isAtLimit ? (
            <Link
              to="/pricing"
              className="flex items-center gap-1.5 px-3 py-2 bg-ember text-white rounded-lg text-xs font-semibold"
            >
              <Zap size={12} /> Upgrade
            </Link>
          ) : (
            !isUnlimited && (
              <div className="text-right">
                <p className="text-lg font-bold font-display">
                  {(hoursLimit - hoursUsed).toFixed(1)}
                </p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-sm">
                Monthly streaming limit reached
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                You've used all {hoursLimit} hours on your {plan?.name} plan.
              </p>
              <Link
                to="/pricing"
                className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 bg-foreground text-background rounded-lg text-sm font-semibold hover:bg-foreground/90 transition-arena-fast"
              >
      ...
      {!channel && (
        <div className="arena-card p-6 text-center">
          <p className="text-muted-foreground mb-3">
            You need a channel before going live.
          </p>
          <Link
            to="/creator-dashboard"
            className="px-5 py-2.5 bg-foreground text-background rounded-lg text-sm font-semibold"
          >
            Set Up Channel
          </Link>
        </div>
            ...
            <div className="arena-card p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div>
                    <p className="font-semibold text-sm">
                      Stream with OBS / RTMP
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Professional quality, requires OBS or streaming software
                    </p>
                  </div>
                </button>
                ...
                >
                  <div className="absolute top-3 right-3">
                    <span className="text-xs font-bold px-2 py-0.5 bg-ember text-white rounded-full">
                      Beta
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Use your phone camera, share a public link instantly
                    </p>
                  </div>
                </button>
              </div>
            ...
            <div className="arena-card p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setPhone("category_id", e.target.value);
                      setPhone("discipline_id", "");
                    }}
                    className="w-full px-3 py-2.5 border border-accessible rounded-lg text-sm focus:outline-none bg-background"
                  >
                    <option value="">Select category</option>
                ...
                <div>
                  ...
                  <select
                    value={phoneForm.discipline_id}
                    onChange={(e) => setPhone("discipline_id", e.target.value)}
                    className="w-full px-3 py-2.5 border border-accessible rounded-lg text-sm focus:outline-none bg-background"
                  >
                    <option value="">Select discipline</option>
                    {phoneFilteredDisciplines.map((d) => (
                      <option key={d.id} value={d.id}>
                        {d.name}
                      </option>
                    ))}
              ...
              {competitions.length > 0 && (
                <div>
                  ...
                  <select
                    value={phoneForm.competition_id}
                    onChange={(e) =>
                      setPhone("competition_id", e.target.value)
                    }
                    className="w-full px-3 py-2.5 border border-accessible rounded-lg text-sm focus:outline-none bg-background"
                  >
                    <option value="">No competition</option>
              ...
              <div className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                ...
                <button
                  ...
                  className={`w-11 h-6 rounded-full transition-arena-fast relative ${
                    phoneForm.micEnabled ? "bg-foreground" : "bg-border"
                  }`}
                >
                  <span
                    className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-arena-fast ${
                      phoneForm.micEnabled ? "left-6" : "left-1"
                    }`}
                  />
                </button>
              </div>
          ...
          {streamMethod === "phone" && localStreamRef.current && !phoneStream && (
            <div className="arena-card p-6 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Camera is live — this preview is only visible to you. Viewers
                will only see the stream after you start.
              </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2 p-3 bg-foreground text-background rounded-lg font-mono text-sm">
                  <span className="flex-1 truncate">
                    {streamCreated.rtmp_url}
                  </span>
                  <button
                    onClick={() => copy(streamCreated.rtmp_url, "url")}
                    className="shrink-0 p-1 hover:opacity-70 transition-arena-fast"
                  >
                    {copiedUrl ? (
              ...
              <div>
                ...
                <div className="flex items-center gap-2 p-3 bg-foreground text-background rounded-lg font-mono text-sm">
                  ...
                  <button
                    onClick={() => copy(streamCreated.stream_key, "key")}
                    className="shrink-0 p-1 hover:opacity-70 transition-arena-fast"
                  >
                    {copiedKey ? (
                      <Check size={14} className="text-emerald-400" />
                ...
                <p className="text-xs text-muted-foreground mt-1.5">
                  ⚠️ Keep your stream key private. Never share it publicly.
                </p>
              </div>
      ...
      {phoneStream && (
        <PhoneBroadcaster
          stream={phoneStream}
          onEndStream={handleEndPhoneStream}
          initialMicEnabled={phoneStream.microphone_enabled}
        />
      )}
    </div>
  );
